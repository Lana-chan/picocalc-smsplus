...
__attribute__((aligned(4))) uint8 bg_pattern_cache[0x20000];/* Cached and flipped patterns */
...
void render_line(int line)
{
	...
	if ((vdp.reg[1] & 0x20)) { // do not draw if display is disabled
		...
		lcd_paletted_draw(&linebuf[bitmap.viewport.x], pixel, bitmap.viewport.off_x, bitmap.viewport.off_y + line, bitmap.viewport.w, 1);
	} else {
		lcd_fill(0, bitmap.viewport.off_x, bitmap.viewport.off_y + line, bitmap.viewport.w, 1);
	}
}
...
void render_bg_sms(int line)
{
	...
	volatile int shift = (hscroll & 7);
	uint32 atex_mask;
	uint32 *cache_ptr;
	volatile uint32 *linebuf_ptr = (uint32 *)(&linebuf[0] - shift);
...
#ifdef PICO_RP2040
		// i don't have a clue why the write_dword below crashes RP2040 and not RP2350, but it does
		uint32 cache[2] = { cache_ptr[0] | atex_mask, cache_ptr[1] | atex_mask };
		memcpy(&linebuf[(0 - shift) + (column << 3)], &cache, 8);
#endif
#ifdef PICO_RP2350
			...
			if(vdp.reg[1] & 0x01)
			{
				...
				for(x = start; x < end; x++)
				{
					...
					if(sp)
					{
						...
						/* Look up result */
						linebuf_ptr[x] = pixel_lut[(bg << 8) | (sp)];
